...
var BINDINGS =
  process.env.OPENGROK_BINDINGS || "/home/box/sand-data/model-bindings.json";
...
function requestedModelId(args) {
  ...
  if (req && req.modelId && typeof req.modelId.modelId === "string")
  ...
  for (var k = 0; k < keys.length; k++) {
    ...
    if (matchesEffortWhen(messages, patterns)) {
      log(
        "effortWhen -> effort=" +
          effort +
          " (patterns matched in tail context)",
      );
...
function dumpProto(stock) {
  ...
  var own =
    stock && typeof stock === "object" ? Object.getOwnPropertyNames(stock) : [];
  var protoKeys =
...
function contentToText(content) {
  ...
  for (var i = 0; i < content.length; i++) {
    ...
    if (typeof p === "string") bits.push(p);
      ...
      bits.push(
        typeof p.result === "string" ? p.result : JSON.stringify(p.result || p),
      );
...
function normalizeHopToolName(name) {
  if (!name) return "";
  var n = String(name);
  if (n === "send_message" || n === "SendMessage" || n === "Send_Message")
    return "SendToUser";
  return n;
}
...
function hopFullStream(
  ...
  var fullStream = (async function* () {
    ...
    try {
      ...
      var msgs =
        typeof exec.getMessages === "function" ? exec.getMessages() : [];
      var turn = {
      ...
      log(
        "stream messages=" +
          ...
          (effLog ? " params=[" + effLog + "]" : ""),
      );
      log(
        "[opengrok-debug] stream start " +
          debugId +
          " inMsgs=" +
          turn.messages.length +
          " tools=" +
      ...
      var out = null;
      var useStream =
        hopSess.streamHop && process.env.OPENGROK_STREAM_HOP !== "0";
        ...
        while (!streamDone || queue.length) {
          if (queue.length) {
            ...
            if (ev.type === "reasoning")
              yield { type: "reasoning", textDelta: ev.textDelta };
      ...
      if (embedded.reasoning) {
        out.reasoning_content =
          ((out && out.reasoning_content) || "") + embedded.reasoning;
      }
      if (embedded.calls.length) {
        calls = calls.concat(embedded.calls);
        text = embedded.text;
        if ((out && out.finish_reason) === "stop")
          ...
          log(
            "[opengrok-debug] raw tool_call " +
              ...
              (typeof rawArgs === "string"
                ? rawArgs.length
                : JSON.stringify(parsedArgs).length),
          );
        }
      }
      log(
        "stream out content=" +
          text.length +
          " reasoning=" +
          ((out && out.reasoning_content) || "").length +
          " tools=" +
          calls.length +
          " embeddedTools=" +
          embedded.calls.length +
          " finish=" +
          ((out && out.finish_reason) || ""),
      );
      if (embedded.calls.length) {
        log(
          "[opengrok-debug] embedded tool calls parsed=" +
            embedded.calls.length +
            " " +
            debugId,
        );
      ...
      var contentParts = Array.isArray(assistantContent)
      ...
      if (!settled.r) {
        ...
        log(
          "[opengrok-debug] response resolved " +
            debugId +
            " finishReason=" +
            finish +
            " contentParts=" +
      ...
      log(
        "[opengrok-debug] emit finish " + debugId + " finishReason=" + finish,
      );
      ...
      log(
        "[opengrok-debug] stream generator error " +
          debugId +
          " " +
          (err && err.message),
      );
